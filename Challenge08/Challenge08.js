...
const getEntries  = (obj) => {
    ...
    var array=[];
 for (const property in obj) {
     let test=`${property}: ${obj[property]}`;
     array.push(test);
...
const getInfo = (arr) => {
  ...
  for(var i=0;i<courses.length;i++){
      ...
      j=0;
      while(j!=courses[i].Students.length){
...
const getStudents = (arr) => {
  ...
  for(var i=0;i<arr.length;i++){
      for(var j=0;j<courses.length;j++){
          m=0;
      while((courses[j].Students[m])!=(courses[j].Students[courses[j].Students.length])){
        if(courses[j].Students[m]===arr[i])
        {
            let array={"Student":arr[i],"course":courses[j]["course"]};
            result.push(array);
            break;
        }
        m++;
